...
- **Status:** CLOSED 2026-07-17 (DDR-0014) — all three write sites (`RTZFunctions`, `RTZParams` via `ListAll`) now compute their exact output extent and check it with a single `Application.WorksheetFunction.CountA` call before writing; if the range already contains anything, a `MsgBox` (Yes/No) confirms before overwriting rather than silently proceeding or permanently refusing. This subsumes the original narrow self-overwrite case (the caller's own formula cell is never "empty") without a separate intersection test. Existing narrow `StrComp` guards left in place, now redundant but harmless. Trade-off (documented in the DDR): a workbook that recalculates one of these formulas automatically (F9, reopen, upstream cell change) will see the confirmation dialog on every recalculation after the first successful write, since the range then always contains the prior output — accepted per explicit user direction favoring a confirm-before-overwrite dialog over silent refusal. `RTZFunctions`' row count (18) is a hand-maintained constant with a comment tying it to the entries below; if it drifts out of sync with a future edit to that list, the guard would check the wrong extent. Build verified clean (0 warnings, both bitness targets pack); Excel-side manual confirmation of the dialog behavior is pending user testing — no automated GUI driver exists for this native Excel COM add-in and a live modal `MsgBox` was not safe to attempt blind.
- **Date recorded:** 2026-07-17
- **Date closed:** 2026-07-17
...
- **Description:** DDR-0015 added a workbook-open update check (`RadToolzAddIn.OnWorkbookOpen`/`CheckForUpdate`) alongside the existing `RTZUpdate()` UDF. Both independently fetch the DNS TXT version record and run the same version-state comparison (newer available / pre-release-of-current). Per `section_09`'s duplication guidance, a second occurrence is duplicated with a recorded debt entry rather than extracted (extraction is for a third occurrence) — this is that entry. Superseded note: `RTZUpdate` was *not* left untouched as originally recorded here — DDR-0016 subsequently converted it to an Excel-DNA async function (`ExcelAsyncUtil.Run`) for unrelated reasons (on-open cell refresh without blocking), moving its logic into a new `ComputeRtzUpdateStatus` helper. The duplication with `CheckForUpdate` persists across that change. It also created a real (now-fixed) side effect: both checks independently detecting the same update meant two dialogs could show. Resolved via a small locked coordination flag (`RadToolzAddIn.ArmDialogSuppression`/`ConsumeDialogSuppression`) rather than by removing the duplication.
- **Risk:** Informational (maintainability only — a version-comparison bug fixed in one copy could be missed in the other).
- **Suggested remedy:** If a third caller ever needs the same version-check logic, extract a shared helper at that point (Rule of Three) rather than duplicating a third time.
- **Date recorded:** 2026-07-17
